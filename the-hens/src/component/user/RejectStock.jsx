...
const RejectStock = () => {
  ...
  const [items, setItems] = useState([
    { item_name: "", quantity: 1, weight: "", reason: "" },
  ]);
  ...
  const handleSubmit = (e) => {
    ...
    for (let item of items) {
      ...
      if (!stockExist || stockExist.Quantity < parseInt(item.quantity)) {
        alert(
          `Insufficient stock for ${item.item_name}. Available: ${stockExist?.Quantity || 0}`,
        );
        return;
      }
    }
        ...
        <div className={styles.container}>
          <div className={styles.headerCard}>
            ...
            <p className="text-gray-500 text-sm mb-4">
              Track and record items removed from inventory via FIFO
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
              className={styles.addRejectBtn}
            >
                ...
                {currentItems.length > 0 ? (
                  currentItems.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td>{item.quantity}</td>
                      <td>
                        <span className={styles.badge}>{item.weight}</span>
                      </td>
                  ...
                  <tr>
                    <td colSpan="5" className="text-center py-10 text-gray-400">
                      No rejected items found for this period
                    </td>
                  </tr>
                )}
              </tbody>
