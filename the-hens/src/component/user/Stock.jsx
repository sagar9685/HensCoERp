  ...
  useEffect(() => {
    if (available.length > 0) {
      ...
      const stats = {
        ...
        lowStockItems: available.filter(item => item.available_stock < 10).length,
        outOfStock: available.filter(item => item.available_stock === 0).length,
        totalQuantity: available.reduce((sum, item) => sum + (item.available_stock || 0), 0)
      };
      setStockStats(stats);
    }
        ...
        <div className="container-fluid page-body-wrapper">
          ...
          <div className={styles.container}>
            ...
            <div className={styles.header}>
              <div className={styles.headerContent}>
                <div className={styles.titleSection}>
                  ...
                  <p className={styles.subtitle}>Real-time stock monitoring & analytics</p>
                </div>
            ...
            <div className={styles.statsGrid}>
              ...
              <div className={styles.statCard}>
                <div className={styles.statIcon} style={{ background: 'linear-gradient(135deg, #00b09b 0%, #96c93d 100%)' }}>
                  <Hash size={24} />
                </div>
                <div className={styles.statContent}>
              ...
              <div className={styles.statCard}>
                <div className={styles.statIcon} style={{ background: 'linear-gradient(135deg, #ff0844 0%, #ffb199 100%)' }}>
                  <AlertCircle size={24} />
                </div>
                <div className={styles.statContent}>
            ...
            <div className={styles.controls}>
              ...
              <div className={styles.filterTags}>
                <span className={`${styles.filterTag} ${styles.tagAll}`}>All Items</span>
                <span className={`${styles.filterTag} ${styles.tagLow}`}>Low Stock</span>
                <span className={`${styles.filterTag} ${styles.tagOut}`}>Out of Stock</span>
                <span className={`${styles.filterTag} ${styles.tagHigh}`}>High Stock</span>
              </div>
            </div>
            ...
            <div className={styles.section}>
              <div className={styles.sectionHeader}>
                <h2 className={styles.sectionTitle}>
                  ...
                  <span className={styles.itemCount}>{filteredStock.length} items</span>
                </h2>
                <div className={styles.stockLegend}>
                  <div className={styles.legendItem}>
                    <span className={`${styles.legendDot} ${styles.highDot}`}></span>
                    High Stock
                  </div>
                  <div className={styles.legendItem}>
                    <span className={`${styles.legendDot} ${styles.mediumDot}`}></span>
                    Medium Stock
                  </div>
                  <div className={styles.legendItem}>
              ...
              {filteredStock.length === 0 ? (
                <div className={styles.emptyState}>
                  ...
                  <p className={styles.emptyMessage}>
                    {searchTerm ? "Try a different search term" : "No stock data available"}
                  </p>
                </div>
              ) : (
                <div className={styles.stockGrid}>
                  {filteredStock.map((item, index) => (
                    <div
                      key={index}
                      className={`${styles.stockCard} ${styles[getStockStatus(item.available_stock)]}`}
                    >
                      <div className={styles.cardHeader}>
                        ...
                        <div className={styles.stockBadge}>
                          <span className={styles.stockQuantity}>{item.available_stock}</span>
                      ...
                      <div className={styles.cardBody}>
                        ...
                        <div className={styles.stockInfo}>
                          <div className={styles.stockDetail}>
                            ...
                            <span className={styles.detailValue}>{item.available_stock} units</span>
                          </div>
                          <div className={styles.stockDetail}>
                            <span className={styles.detailLabel}>Status:</span>
                            <span className={`${styles.statusBadge} ${styles[getStockStatus(item.available_stock)]}`}>
                      ...
                      <div className={styles.cardFooter}>
                        <div className={styles.stockIndicator}>
                          <div className={styles.indicatorBar}>
                            <div 
                              className={`${styles.indicatorFill} ${styles[getStockStatus(item.available_stock)]}`}
                              style={{ width: `${Math.min(item.available_stock * 2, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
