...
//  const handlePrint = () => {
//   // Chhota sa delay taaki UI stable ho jaye
//   setTimeout(() => {
//     window.print();
//   }, 100);
// };
        ...
        <div className={styles.filterBar}>
          ...
          <div className={styles.inputGroup}>
            ...
            <select 
              ...
              onChange={(e) => setFilters({...filters, deliveryBoyId: e.target.value})}
            >
              <option value="all">Full Shop (All)</option>
              {/* 3. Mapping real delivery men from Redux */}
              {deliveryMen && deliveryMen.map((boy) => (
  ...
  disabled={dailyLoading} // Data load hote waqt disable rakhein
>
  <Printer size={18} /> {dailyLoading ? "Loading..." : "Print Report"}
</button> */}
        </div>
      </div>
        ...
        <>
          ...
          <div className={styles.statsGrid}>
            <div className={`${styles.statCard} ${styles.blue}`}>
              ...
              <h3>₹{Number(daily.summary?.totalSaleAmount || 0).toLocaleString()}</h3>
            </div>
            <div className={`${styles.statCard} ${styles.green}`}>
              <span className={styles.cardLabel}>Payment Collected</span>
              <h3>₹{Number(daily.summary?.totalReceived || 0).toLocaleString()}</h3>
            </div>
            <div className={`${styles.statCard} ${styles.red}`}>
              <span className={styles.cardLabel}>Outstanding Balance</span>
              <h3>₹{Number(daily.summary?.totalOutstanding || 0).toLocaleString()}</h3>
            </div>
          </div>
          ...
          <div className={styles.reportContent}>
            ...
            <div className={styles.tableSection}>
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {daily.products?.length > 0 ? daily.products.map((item, idx) => (
                    <tr key={idx}>
                      <td className={styles.fontMedium}>{item.ProductType}</td>
                      <td>{item.Weight || '-'}</td>
                      <td>{item.Qty}</td>
                      <td>₹{item.Rate}</td>
                    ...
                    <tr><td colSpan="5" className={styles.noData}>No orders found for this selection.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className={styles.tableSection}>
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {daily.payments?.length > 0 ? daily.payments.map((pay, idx) => (
                    <tr key={idx}>
                      <td>{pay.ModeName}</td>
                      <td className={`${styles.textRight} ${styles.bold} ${styles.greenText}`}>
                        ₹{Number(pay.ModeTotal).toLocaleString()}
                      </td>
                    </tr>
                  )) : (
                    <tr><td colSpan="2" className={styles.noData}>No payments recorded.</td></tr>
                  )}
                </tbody>
